// Installaion live-server pour simuler serveur avec url type serveur
// Installation json-server pour simuler API Rest (db.json utilisé comme API Rest)

console.log('hello depuis main');
const technosDiv = document.querySelector('#technos');

function loadTechnologies() {
	// fetch() sur url retourne promesse
	fetch('http://localhost:3001/technos')
		.then(response => {
			response.json() // méthode .json() retourne promesse qui doit être résolue pour atteindre les technos
				.then(technos => {
